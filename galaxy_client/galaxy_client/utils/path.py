...
def makedirs_safe(path, mode=None):
    ...
    :raises GalaxyClientError: If the directory cannot be created and does not already exists.
    ...
    if not os.path.exists(b_rpath):
        try:
            ...
            if e.errno != EEXIST:
                raise exceptions.GalaxyClientError("Unable to create local directories(%s): %s" % (to_native(rpath), to_native(e)))
