...
if runtime.ANSIBLE_FORCE_COLOR:
    ANSIBLE_COLOR = True
...
def hostcolor(host, stats, color=True):
    if ANSIBLE_COLOR and color:
        if stats['failures'] != 0 or stats['unreachable'] != 0:
            return u"%-37s" % stringc(host, runtime.COLOR_ERROR)
        elif stats['changed'] != 0:
            return u"%-37s" % stringc(host, runtime.COLOR_CHANGED)
        else:
            return u"%-37s" % stringc(host, runtime.COLOR_OK)
    return u"%-26s" % host
